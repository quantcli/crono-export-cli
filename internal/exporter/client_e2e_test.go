...
// that exporter.NewLoggedIn honors.
...
// fixedRange is a stable test window; exporter just forwards Start/End
// to cronoapi, so the dates don't need to match anything in the canned CSV.
func fixedRange() exporter.DateRange {
	return exporter.DateRange{
...
func TestNewLoggedIn_RequiresCredentials(t *testing.T) {
	...
	if _, err := exporter.NewLoggedIn(context.Background()); err == nil {
...
func TestNewLoggedIn_HonorsBaseURLEnv(t *testing.T) {
	f := withFake(t)
	c, err := exporter.NewLoggedIn(context.Background())
	if err != nil {
