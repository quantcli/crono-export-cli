...
during a full crono-export-cli session. It's the wire-level reference
for `internal/cronoapi` — recapture against a fresh session whenever
Cronometer changes a wire shape.
...
as their first argument). A simple correctness check: the token
extracted here should match the one step 14 (`logout`) sends in its
body.
...
account data and are safe to commit). The typed-record field names
live in `internal/cronoapi/records.go`; a fresh header capture can be
diffed against the `<Name><Unit>` convention there to confirm the
column-name mapping.
...
Open questions for future captures:
